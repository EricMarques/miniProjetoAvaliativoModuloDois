...
SELECT
    ...
    CASE WHEN TRIM(REPLACE(p.`QTD.Itens`, '-', '')) = '' THEN NULL
...
LEFT JOIN dim_loja dl ON dl.chave_loja = UPPER(TRIM(REPLACE(REPLACE(p.`Loja-Nome`, '/SC', ''), '  ', ' ')))
            OR (UPPER(TRIM(REPLACE(p.`Loja-Nome`, '/SC', ''))) = 'PATA AMIGA BLUMENAL CENTRO' AND dl.chave_loja = 'PATA AMIGA BLUMENAU CENTRO')
            OR (UPPER(TRIM(REPLACE(p.`Loja-Nome`, '/SC', ''))) = 'PATA AMIGA FLORIPA NORTE' AND dl.chave_loja = 'PATA AMIGA FLORIANOPOLIS NORTE')
            OR (UPPER(TRIM(REPLACE(p.`Loja-Nome`, '/SC', ''))) = 'PATA AMIGA JGUA DO SUL' AND dl.chave_loja = 'PATA AMIGA JARAGUA DO SUL')
